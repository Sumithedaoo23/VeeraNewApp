...
app.whenReady().then(() => {
  ...
  ipcMain.handle('send-class', async (event, cls) => {
    console.log('[App→Kit] send-class', cls);
    try {
      serialManager.send(`#C:${cls}$`);
      return 'ok';
    } catch (e) {
      console.error('Failed to send class', e);
      return 'fail';
    }
  });

  ipcMain.handle('send-experiment', async (event, exp) => {
    console.log('[App→Kit] send-experiment', exp);
    try {
      serialManager.send(`#E:${exp}$`);
      return 'ok';
    } catch (e) {
      console.error('Failed to send experiment', e);
      return 'fail';
    }
  });
