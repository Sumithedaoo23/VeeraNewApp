...
contextBridge.exposeInMainWorld('veeraAPI', {
    // high level commands
  sendClass: (c) => {
    console.log('[preload] sendClass ->', c);
    ipcRenderer.invoke('send-class', c);
    return ipcRenderer.invoke('send-class', c); // also invoke if main expects it
  },
  sendExperiment: (e) => {
    console.log('[preload] sendExperiment ->', e);
    ipcRenderer.invoke('send-experiment', e);
    return ipcRenderer.invoke('send-experiment', e);
  },
  sendThresholdsFor: (key) => {
    console.log('[preload] sendThresholdsFor ->', key);
    ipcRenderer.invoke('send-thresholds-for', key);
  },
